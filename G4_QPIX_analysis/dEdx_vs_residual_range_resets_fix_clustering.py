...
def reconstruct_track_segments(contributions, bin_width_cm, recombination_enabled):
    ...
    for (event, track_id), track in contributions.groupby(group_columns, sort=False):
        ...
        # CLUSTERING FIX: Keep bins anchored at the reconstructed track start,
        # exactly as in the original script, so different track lengths retain
        # their natural residual-range offsets instead of forming vertical bands.
        full_bin_count = int(np.floor(track_length / bin_width_cm))
        # CLUSTERING FIX: Measure the short distance left after the full bins.
        endpoint_remainder = track_length - full_bin_count * bin_width_cm
        # CLUSTERING FIX: Treat floating-point roundoff at an exact bin boundary
        # as zero remainder so a spurious tiny endpoint interval is not created.
        has_endpoint_remainder = endpoint_remainder > 1.0e-12

        # CLUSTERING FIX: Build explicit (low, high) intervals along the forward
        # track coordinate. Each reset will belong to one and only one interval.
        intervals = []
        if full_bin_count == 0:
            # CLUSTERING FIX: A track shorter than one requested bin is represented
            # by one interval covering its complete fitted length.
            intervals.append((0.0, track_length))
        elif has_endpoint_remainder:
            # CLUSTERING FIX: Preserve every ordinary full bin except the one
            # immediately before the short endpoint remainder.
            for bin_number in range(full_bin_count - 1):
                low = bin_number * bin_width_cm
                high = (bin_number + 1) * bin_width_cm
                intervals.append((low, high))
            # CLUSTERING FIX: Merge the final full bin and short remainder into
            # one endpoint interval whose width is between one and two bin widths.
            merged_low = (full_bin_count - 1) * bin_width_cm
            intervals.append((merged_low, track_length))
        else:
            # CLUSTERING FIX: Exact multiples need no merge; retain every normal
            # full-width bin through the reconstructed endpoint.
            for bin_number in range(full_bin_count):
                low = bin_number * bin_width_cm
                high = (bin_number + 1) * bin_width_cm
                intervals.append((low, high))

        for interval_number, (low, high) in enumerate(intervals):
            # CLUSTERING FIX: Use the interval's true length, including the wider
            # merged endpoint interval, as dx in the dE/dx calculation.
            path_length = high - low
            # CLUSTERING FIX: Assign each reset to its single forward-track bin.
            in_interval = (projection >= low) & (projection < high)
            # CLUSTERING FIX: Include the fitted endpoint in the final interval;
            # all other upper edges remain exclusive to prevent double counting.
            if interval_number == len(intervals) - 1:
                in_interval |= np.isclose(projection, track_length)
            if not np.any(in_interval):
                continue
            interval_weights = weights[in_interval]
            electron_sum = float(interval_weights.sum())
            contribution_count = int(np.count_nonzero(in_interval))
            ...
            if not np.isfinite(dedx) or dedx <= 0.0:
                continue
            # CLUSTERING FIX: Convert the forward interval midpoint to residual
            # range. Track-dependent lengths keep these positions distributed.
            midpoint = 0.5 * (low + high)
            residual_range = track_length - midpoint
            rows.append(
                {
                    "event": int(event),
                    ...
                    "residual_range_cm": residual_range,
                    # CLUSTERING FIX: Save both interval edges so the exact
                    # spatial support of every dE/dx estimate remains auditable.
                    "track_bin_low_cm": low,
                    "track_bin_high_cm": high,
                    ...
                    "n_reset_contributions": contribution_count,
                }
            )
...
def main():
    ...
    # CLUSTERING FIX: Use a separate folder so this experiment cannot overwrite
    # any products made by the original reconstruction script.
    output_dir = SCRIPT_DIR / "dEdx_resets" / f"kaon_decay_{number_events}_events_fix_clustering"
    output_dir.mkdir(parents=True, exist_ok=True)
    bin_width_tag = f"binwidth{args.bin_width_cm:g}cm".replace(".", "p")
    # CLUSTERING FIX: Tag every PNG as well as the folder so individual files
    # remain unmistakable if they are copied elsewhere for comparison.
    primary_plot = output_dir / f"primary_kaons_resets_dEdx_vs_residual_range_{number_events}_events_{bin_width_tag}_mergedendpoint_fix_clustering.png"
    primary_zoom_plot = output_dir / f"primary_kaons_resets_dEdx_vs_residual_range_xmax10cm_ymax40MeVcm_{number_events}_events_{bin_width_tag}_mergedendpoint_fix_clustering.png"
    all_plot = output_dir / f"all_particles_resets_dEdx_vs_residual_range_{number_events}_events_{bin_width_tag}_mergedendpoint_fix_clustering.png"
    all_zoom_plot = output_dir / f"all_particles_resets_dEdx_vs_residual_range_xmax10cm_ymax40MeVcm_{number_events}_events_{bin_width_tag}_mergedendpoint_fix_clustering.png"
    ...
    summary_path.write_text(
        "\n".join(
            [
                ...
                f"nominal bin width [cm]: {args.bin_width_cm}",
                "bin alignment: forward from the fitted track start",
                "endpoint treatment: merge the remainder into the preceding full bin",
            ]
        )
        + "\n"
    )
