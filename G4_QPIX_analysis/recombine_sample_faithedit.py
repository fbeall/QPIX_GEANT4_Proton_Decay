...
# Recombine the per-particle end-of-life CSVs written by root_to_csv_faithedit.py. (FB 8/17/26)
particles = pd.read_csv(str('%s/particles_E0.txt'%(pwd)),index_col=0)
...
for event in np.arange(1,nEvents):
	...
	# Add the matching per-particle final-position, decay, and detector-tag data. (FB 8/17/26)
	filename3 = str('%s/particles_E%d.txt'%(pwd,event))
	df3 = pd.read_csv(filename3)
	particles = pd.concat([particles,df3],ignore_index=True)

resets.to_csv(str('%s/resets_output.txt'%pwd))
g4.to_csv(str('%s/g4_output.txt'%pwd))
# Write the recombined particle end-of-life table as its own output. (FB 8/17/26)
particles.to_csv(str('%s/particles_output.txt'%pwd))
